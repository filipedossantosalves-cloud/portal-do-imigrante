...
for (const service of publicData.services) {
  ...
  if (service.sourceRegistered) {
    ...
    if (!/^https:\/\//i.test(service.sourceUrl || "")) errors.push(`${service.id}: official record without HTTPS source URL`);
...
for (const relativePath of ["index.html", "research-desk/index.html", "data/index.html"]) {
...
for (const filePath of files.filter((item) => textExtensions.has(path.extname(item).toLowerCase()))) {
  ...
  if (/sk-[A-Za-z0-9_-]{16,}|ghp_[A-Za-z0-9]{20,}|BEGIN (?:RSA |OPENSSH )?PRIVATE KEY/i.test(content)) {
